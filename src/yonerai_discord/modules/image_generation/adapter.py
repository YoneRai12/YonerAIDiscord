...
_UNAVAILABLE_MESSAGE = "画像生成は現在利用できません。provider設定と安全条件を確認してください。"


class ImageGroup(app_commands.Group):
    def __init__(self, adapter: DiscordImageGenerationAdapter) -> None:
        self.adapter = adapter
        super().__init__(name="image", description="明示的に画像生成を実行します")

    @app_commands.command(name="generate", description="設定済みproviderでPNG画像を1枚生成します")
...
class DiscordImageGenerationAdapter:
    def __init__(
        self,
        ...
        *,
        ...
        if not isinstance(service, ImageGenerationService):
            raise TypeError("service must be an ImageGenerationService")
        if not callable(capability_check):
            raise TypeError("capability_check is required")
        self.service = service
        self.capability_check = capability_check
        self.group = ImageGroup(self)
        self._closing = False
    ...
    async def generate(
        ...
        try:
            request = ImageGenerationRequest(
                request_id=f"image-{int(interaction.id)}",
        ...
        except (ImageGenerationAuthorizationError, ImageGenerationError, TypeError, ValueError):
        ...
        embed = discord.Embed(
            title="生成画像",
            description="検証済みPNGを添付しました。",
            color=0x5865F2,
        )
        embed.set_image(url=f"attachment://{GENERATED_IMAGE_FILENAME}")
        await interaction.followup.send(
            embed=embed,
            file=discord.File(BytesIO(generated.png), filename=GENERATED_IMAGE_FILENAME),
            ephemeral=True,
            allowed_mentions=discord.AllowedMentions.none(),
        )
    ...
    async def generate_for_message(
        ...
        *,
        ...
        try:
            request = ImageGenerationRequest(
                request_id=f"image-mention-{message_id}",
    ...
    async def _allowed(self, interaction: discord.Interaction) -> bool:
        if self._closing or interaction.guild_id is None:
            return False
        try:
            result = self.capability_check(IMAGE_GENERATION_CAPABILITY_ID, interaction)
...
__all__ = ["DiscordImageGenerationAdapter", "ImageGroup"]
